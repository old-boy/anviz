...
require('../models/ProductCateogoryTwoSchema');
const CateogoryOneSchema = mongoose.model('cateogories');
const CateogoryTwoSchema = mongoose.model('twoCateogories');
 ...
 * POST /cateogory/add/:id  根据第一级id添加第二级cateogory
 */
...
router.post('/cateogory/add/:id',urlencodedParser,(req,res) => {
      let errors = [];
      ...
      if(errors.length > 0){
            res.render('/cateogory/add',{
                  parentCateogoryName: req.body.cateogoryName,
                  cateogoryName:req.body.cateogoryTwoName,
                  _id:req.params.id
            })
      }else{
            const newFields = {
                  cateogoryName: req.body.cateogoryOneName,
                  cateogoryDes:req.body.cateogoryOneDes,
                  parentCateogoryId:req.params.id
            }

            console.log('newTwoFields    ' + newFields)
            CateogoryTwoSchema.findOne({cateogoryName:newFields.cateogoryName}).then(cateogoryName => {
                  if(cateogoryName){
                        req.flash("error_msg","数据库中存在相同数据，不可继续添加");
                        res.redirect('/product/cateogory');
                  }else{
                        new CateogoryTwoSchema(newFields).save().then(cateogories => {
                              req.flash("success_msg","数据编辑成功!");
                              res.redirect('/product/cateogory');
                        });
                  }
